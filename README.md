...
I wrote a Tampermonkey userscript that adds cumulative **Income vs. Spend** and group-based **Spending Trends** above YNAB's native monthly bar chart. These visuals show how income and spending trends build through the year, making trends easier to identify. The category selector lets you exclude savings and investment categories from the Spend total, so they don't mask your real spending patterns. The spending trends view was motivated by my desire to watch lifestyle creep in key categories, such as spending on eating out and personal services.

> The illustrations below use mock sample data 
...
- **Group and category picker.** One dropdown, styled after YNAB's own "All Categories" selector, lets you choose exactly which categories and groups count toward each chart — both individual categories and whole groups are selectable in the same list.
- **Saved selections, per chart and per budget.** Income vs. Spend and Spending Trends each remember their own selection. Pick your categories, then click **Save** in the picker to persist them for that budget; **Cancel** (or clicking outside the panel) discards unsaved changes, and **Reset** clears the in-progress selection before you save.
...
6. The Income vs. Spend and Spending Trends charts are inserted above the native bar chart. Each chart has its own category picker; choices are stored per budget when you click **Save** within that selector.
...
To change or clear the stored token, use **Reset YNAB API Token** from the Tampermonkey extension menu (click the Tampermonkey icon while on a YNAB tab). A rejected token (i.e. after revoking a token in YNAB) will be cleared automatically. You will be re-prompted to enter a fresh API token on your next visit.
...
The script calls YNAB's public <a href="https://api.ynab.com/">API</a> directly from your browser, so there's no server involved. Data is fetched once per year you select, then all the cumulative math and chart rendering happens client-side. Your token stays in Tampermonkey's local storage.
